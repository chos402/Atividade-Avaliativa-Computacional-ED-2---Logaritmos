...
/* ================= CONTROLAR FILTRO ESPECIALIDADE ================= */
...
function atualizarDashboard() {
    ...
    // Limpa todos os cards antes de recriar
    cardsContainer.innerHTML = "";

    /* ================= FILTROS ================= */

    const medicosAtivos = profissionais.filter(p =>
        p.cargo === "Médico" && p.status === "Ativo"
    );

    const enfermeirosAtivos = profissionais.filter(p =>
        p.cargo === "Enfermeiro" && p.status === "Ativo"
    );

    /* ================= CARD MÉDICOS ================= */

    const cardMedicos = document.createElement("div");
    cardMedicos.classList.add("card");

    cardMedicos.innerHTML = `
        <h3>Lista de Médicos</h3>
        <div class="lista-medicos"></div>
        <div class="paginacao-dashboard"></div>
    `;

    cardsContainer.appendChild(cardMedicos);

    const listaMedicosDiv = cardMedicos.querySelector(".lista-medicos");
    const paginacaoDiv = cardMedicos.querySelector(".paginacao-dashboard");

    const itensPorPaginaDash = 4;
    let paginaAtualDash = 1;

    function renderizarMedicos() {

        const totalPaginas = Math.ceil(medicosAtivos.length / itensPorPaginaDash) || 1;

        if (paginaAtualDash > totalPaginas) paginaAtualDash = 1;

        const inicio = (paginaAtualDash - 1) * itensPorPaginaDash;
        const fim = inicio + itensPorPaginaDash;
        const medicosPagina = medicosAtivos.slice(inicio, fim);

        listaMedicosDiv.innerHTML = "";

        if (medicosPagina.length === 0) {
            listaMedicosDiv.innerHTML = "<p>Nenhum médico ativo cadastrado.</p>";
        }

        medicosPagina.forEach(m => {

            const foto = m.foto || IMAGEM_PADRAO;

            listaMedicosDiv.innerHTML += `
                <div class="dashboard-card-medico">
                    <img src="${foto}" 
                         class="dashboard-foto"
...
<div class="telefone">
    ${m.telefone}
        ...
        if (totalPaginas > 1) {
            paginacaoDiv.innerHTML = `
                <button ${paginaAtualDash === 1 ? "disabled" : ""} id="prevDash">⬅</button>
                <button ${paginaAtualDash === totalPaginas ? "disabled" : ""} id="nextDash">➡</button>
            `;

            document.getElementById("prevDash").onclick = () => {
                paginaAtualDash--;
                renderizarMedicos();
            };

            document.getElementById("nextDash").onclick = () => {
                paginaAtualDash++;
                renderizarMedicos();
            };
        }
    ...
    const cardEnfermeiros = document.createElement("div");
    cardEnfermeiros.classList.add("card");
    cardEnfermeiros.style.marginTop = "40px";

    cardEnfermeiros.innerHTML = `
        <h3>Lista de Enfermeiros</h3>
        <div class="lista-enfermeiros"></div>
    `;

    cardsContainer.appendChild(cardEnfermeiros);

    const listaEnfermeirosDiv = cardEnfermeiros.querySelector(".lista-enfermeiros");

    if (enfermeirosAtivos.length === 0) {
        listaEnfermeirosDiv.innerHTML = "<p>Nenhum enfermeiro ativo cadastrado.</p>";
    } else {

        enfermeirosAtivos.forEach(e => {

            const foto = e.foto || IMAGEM_PADRAO;

            listaEnfermeirosDiv.innerHTML += `
                <div class="dashboard-card-medico">
                    <img src="${foto}" 
                         class="dashboard-foto"
                         onerror="this.src='${IMAGEM_PADRAO}'">
